//Taka's added code for PlacedOrderQuery.java.
function cbn(){
	alert("name");
}
...
/*function deleteData(){
	...
	var instr=getClass().getResourceAsStream("dbConn.properties");
	props.load(instr);
	instr.close();
	//var conn=DriverManager.getConnection("jdbc:")
	var driver=props.getProperty("driver.name");
	var url=props.getProperty("server.name");
	var username=props.getProperty("user.name");
	var passwd=props.getProperty("user.password");
	Class.forName(driver);
	conn=DriverManager.getConnection(url,username,passwd);
	//conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","TAKANIMO","0429Taka");
	var query1="UPDATE javafinal.placed_order SET FINISHED=NOT FINISHED";
	//WHERE IRUKAMO
	ps=conn.prepareStatement(query1);
	results=ps.executeQuery();
	var query2="DELETE FROM javafinal.placed_order WHERE FINISHED=TRUE";	
